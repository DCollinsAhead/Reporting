// Point this at the deployed proxy before publishing to Roost. Can be
// overridden per-load with ?api=https://your-proxy for testing.
const DEFAULT_API_BASE_URL = 'http://localhost:3000';
const REFRESH_INTERVAL_MS = 15 * 60 * 1000;

const params = new URLSearchParams(window.location.search);
const API_BASE_URL = params.get('api') || DEFAULT_API_BASE_URL;
...
async function loadStatusSummary() {
  ...
  try {
    ...
    console.error(err);
    setSectionStatus(statusEl, 'Could not load status summary from the proxy.', true);
    return null;
  }
}
...
async function loadAssigneeWorkload() {
  ...
  try {
    ...
    console.error(err);
    setSectionStatus(statusEl, 'Could not load assignee workload from the proxy.', true);
    return null;
  }
}
